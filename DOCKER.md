...
A plugin that routes slash commands directly to sub-agents, bypassing the main (expensive) model:
- `/GE <message>` — Send to a cheaper agent (e.g. Gemini)
- `/eng <message>` — Send to Engineer agent
- `/persona on|off` — Toggle persona mode (optional)
...
**Prerequisite:** You need sub-agents configured in your OpenClaw setup. The plugin routes commands to agents by their ID (e.g. `ge`, `engineer`). If those agents don't exist, the commands will fail.
...
4. Test with `/GE hello` in the webchat
...
| `COMMAND_ROUTER_AGENTS` | `GE:ge:...:120,eng:engineer:...:300` | Agent definitions (see format below) |
...
```bash
# Default (two agents):
COMMAND_ROUTER_AGENTS="GE:ge:Gemini agent:120,eng:engineer:Engineer agent:300"

# Single cheap agent:
COMMAND_ROUTER_AGENTS="quick:haiku:Fast cheap agent:60"

# Three agents:
COMMAND_ROUTER_AGENTS="GE:ge:Gemini:120,eng:engineer:Engineer:300,fast:sonnet:Sonnet agent:90"
```
